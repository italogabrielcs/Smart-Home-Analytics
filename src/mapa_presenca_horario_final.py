...
# Filtrar por data e estado 'on'
dados_data = [d for d in dados if d['date'] == primeira_data and d['new_state'] == 'on']

# Sensores de presença
sensor_count = Counter(d['entity_id'] for d in dados_data)
principais_sensores = sensor_count.most_common()

print(f"\nSensores de presença na data {primeira_data}:")
...
<html>
<head>
    <title>Mapa de Presença por Horário - {primeira_data}</title>
    <style>
        ...
        .heatmap {{ display: grid; grid-template-columns: 250px repeat(24, 30px); gap: 2px; margin: 20px 0; }}
        .sensor-label {{ font-size: 11px; font-weight: bold; padding: 5px; text-align: right; display: flex; align-items: center; justify-content: end; }}
...
<body>
    <div class="container">
        <h1>🏠 Mapa de Presença por Horário</h1>
        <h2>📅 {primeira_data}</h2>
...
for sensor, _ in principais_sensores:
    ...
    nome_simples = sensor.replace('binary_sensor.', '').replace('_', ' ')[:35]
...
html += f"""<p><strong>Total detecções:</strong> {total_eventos}</p>
                <p><strong>Sensores de presença:</strong> {sensores_unicos}</p>
                <p><strong>Horário pico:</strong> {hora_pico[0]:02d}:00 ({hora_pico[1]} detecções)</p>
                <p><strong>Sensor mais ativo:</strong> {sensor_mais_ativo[0][:40]} ({sensor_mais_ativo[1]} detecções)</p>
            </div>
...
for periodo, count in periodos.items():
    ...
    html += f'<p><strong>{periodo}:</strong> {count} detecções ({percentual:.1f}%)</p>'
...
with open('mapa_presenca_horario.html', 'w', encoding='utf-8') as f:
    f.write(html)

print(f"\nEstatísticas da data {primeira_data}:")
print(f"Total de detecções: {total_eventos}")
print(f"Número de sensores de presença: {sensores_unicos}")
print(f"Horário com mais atividade: {hora_pico[0]:02d}:00")
print(f"Sensor mais ativo: {sensor_mais_ativo[0]} ({sensor_mais_ativo[1]} detecções)")

print(f"\nDistribuição por período do dia:")
for periodo, count in periodos.items():
    percentual = (count / total_eventos * 100) if total_eventos > 0 else 0
    print(f"{periodo}: {count} detecções ({percentual:.1f}%)")

print(f"\nMapa HTML gerado: mapa_presenca_horario.html")